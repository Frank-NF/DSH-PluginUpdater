...
/// 返回 (解析后的响应, 签名验证结果: Some=true, Some=false, None=无签名头)
pub async fn verify_page_signature(
    client: &reqwest::Client,
    url: &str,
) -> AppResult<(WebsiteCatalogResponse, Option<bool>)> {
    ...
    // 先提取签名头，再读取响应体
    let sig_header = resp.headers().get("X-DSH-SIGNATURE").cloned();
    let body_bytes = resp.bytes().await?;
    let parsed: WebsiteCatalogResponse = serde_json::from_slice(&body_bytes)?;

    // 提取并验证签名
    if let Some(sig_header) = sig_header {
        let sig = sig_header.to_str().unwrap_or("");
        let body_str = String::from_utf8_lossy(&body_bytes);
        let sig_valid = verify_catalog_signature(sig, &body_str, SIGNING_PUB_KEY);
        return Ok((parsed, Some(sig_valid)));
    }
