...
func main() {
	...
	r.Use(cors.New(cors.Config{
		...
		AllowHeaders:     []string{"Origin", "Content-Type", "X-Proxy-Token", "X-DSH-SIGNATURE"},
		ExposeHeaders:    []string{"Content-Length", "Content-Disposition", "X-DSH-SIGNATURE"},
...
func getPluginList(c *gin.Context) {
	...
	if cached := getCache(cacheKey); cached != "" {
		// 添加签名头
		if sig := signData([]byte(cached)); sig != "" {
			c.Header("X-DSH-SIGNATURE", sig)
			c.Header("X-DSH-PUBKEY", getPublicKeyHex())
		}
